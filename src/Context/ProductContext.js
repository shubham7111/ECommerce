...
const ProductContext = ({children}) => {
    ...
    const filteredSearchData = state?.search.length > 0 ? 
    state?.products.filter((item) => item.title.toLowerCase().includes(state?.search.toLowerCase()))
    : state?.products 

    const filteredSortData = state?.sort.length > 0 ?

    filteredSearchData.sort((a,b) => state?.sort === "lowtohigh" ? a.price - b.price:  b.price - a.price)
    : 
    filteredSearchData
    ...
    const valuetobepassed = {setInputText, dispatch, state, filteredSearchData,filteredCategoryData, state, filteredDataGenre }
    return (
        <ProductKey.Provider value = {valuetobepassed}>{children}</ProductKey.Provider>
    )
}
