...
export default function Payment() {
    ...
    const totalProductsCount = state?.cart?.reduce((acc, item) => acc += item?.qty , 0)
    ...
    return (
        ...
        <div className="address">
       {
                state?.address?.map((addresss, i) => 
                    <div key = {i}>
                    <h3> Address {i+1}</h3>
                    <input type="radio" name = "address" value={addresss.name} onChange={selectHandler}/> <p><strong>{addresss.name}</strong></p>
                   
                    <p><strong>Address: </strong> {addresss.area}</p>
                    <p><strong>Phone: </strong> {addresss.phone}</p>
                    {/* {console.log(i)} */}
        ...
        <div className="payment">
           <p><hr/><strong>Order details</strong><hr/></p>
...
<p><hr/><strong>Deliver To</strong><hr/></p>
<p><strong>{address.name}</strong>
<br/>
{address.area}
<br/>{address.city}<br/>{address.state}<br/>{address.phone}</p>
        <p></p>
        <p></p>
        <p></p>
